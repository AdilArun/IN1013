--1
SELECT DISTINCT first_name, surname FROM restBill JOIN restStaff ON waiter_no = staff_no WHERE cust_name = 'Tanya Singh';

--2
SELECT restRoom_management.room_date FROM restRoom_management JOIN restStaff ON restRoom_management.headwaiter = restStaff.staff_no WHERE restRoom_management.room_name = 'Green'AND restStaff.first_name = 'Charles' AND restRoom_management.room_date BETWEEN 160201 AND 160229;

--3
SELECT restStaff.first_name, restStaff.surname FROM restStaff WHERE restStaff.headwaiter = ( SELECT restStaff.headwaiter FROM restStaff WHERE restStaff.first_name = 'Zoe' AND restStaff.surname = 'Ball');
